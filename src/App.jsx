                    ...
                    {page === 'chat' && (
                        <div className="space-y-1 pt-4 border-t border-white/10">
                            <div className="flex justify-between items-center px-2 mb-2">
                                <div className="text-xs font-bold opacity-40 uppercase tracking-widest">History</div>
                                <button onClick={onNewChat} className="p-1 hover:bg-white/10 rounded"><Plus size={14}/></button>
                            </div>
                            {chats.map(chat => (
...
export default function App() {
  ...
  const handleSend = async (e) => {
      ...
      try {
          ...
          const res = await fetch(`${BACKEND_URL}/api/v1/chat`, { method: 'POST', body: formData });
